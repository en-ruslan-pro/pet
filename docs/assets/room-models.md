...
Страница `/demo` использует модели `frames`, `couch`, `armchair`, `rug`, `simple_library_A`, `lamp` и `plant`. Они формируют постоянный интерьер; пол и стены остаются геометрией сцены. Модель `frames` используется как декоративные рамки. Миски, когтеточка, мяч и игрушечная мышь построены в Three.js, так как соответствующих моделей в наборе нет.

Позиции, размеры и проходимость предметов заданы в `resources/js/room-layout.js`. Предмет с `catCanEnter: true` не блокирует маршрут кошки; например, ковёр и миски остаются доступными.

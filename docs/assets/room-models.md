...
Страница `/demo` использует модели `frames`, `shelf`, `simple_desk_A`, `simple_chair`, `couch`, `armchair`, `simple_library_A`, `lamp` и `plant`. Они заменяют процедурные заглушки интерьера; пол и стены остаются геометрией сцены. Модель `frames` используется как декоративные рамки, а `shelf` — как настенные полки.

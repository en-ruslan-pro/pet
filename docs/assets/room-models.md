...
Страница `/demo` использует модели `frames`, `couch`, `armchair`, `simple_single_bed`, `rug`, `simple_library_A`, `lamp` и `plant`. Они формируют постоянный интерьер; пол и стены остаются геометрией сцены. Модель `frames` используется как декоративные рамки. Миски, когтеточка, окно, мяч и игрушечная мышь построены в Three.js, так как соответствующих моделей в наборе нет.

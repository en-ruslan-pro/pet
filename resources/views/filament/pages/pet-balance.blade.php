<x-filament-panels::page>
    <div class="grid gap-6">
                    ...
                    <tbody>
                        @forelse ($actionRows as $row)
                            <tr class="border-b border-gray-100 dark:border-white/10">
                                ...
                                <td class="px-2 py-2">{{ $row['totalMilliseconds'] }} ms</td>
                                <td class="px-2 py-2">{{ $row['averageMilliseconds'] }} ms</td>
                            </tr>
                        @empty
                            <tr><td class="px-2 py-3" colspan="6">{{ __('pet.analytics.no_data') }}</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        <x-filament::section :heading="__('pet.analytics.needs')">
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm">
                    <thead class="border-b text-gray-600 dark:text-gray-400">
                        <tr><th class="px-2 py-2">{{ __('pet.analytics.recorded_at') }}</th><th class="px-2 py-2">{{ __('pet.needs.satiety') }}</th><th class="px-2 py-2">{{ __('pet.needs.energy') }}</th><th class="px-2 py-2">{{ __('pet.needs.happiness') }}</th></tr>
                    </thead>
                    <tbody>
                        @forelse ($needSnapshots as $snapshot)
                            <tr class="border-b border-gray-100 dark:border-white/10"><td class="px-2 py-2">{{ $snapshot->recorded_at?->format('Y-m-d H:i') }}</td><td class="px-2 py-2">{{ $snapshot->satiety }}</td><td class="px-2 py-2">{{ $snapshot->energy }}</td><td class="px-2 py-2">{{ $snapshot->happiness }}</td></tr>
                        @empty
                            <tr><td class="px-2 py-3" colspan="4">{{ __('pet.analytics.no_data') }}</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
